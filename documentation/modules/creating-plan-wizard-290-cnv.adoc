...
[id="creating-plan-wizard-290-cnv_{context}"]
...
* Have an {virt} source provider and an {virt} destination provider. For more information, see xref:adding-source-provider_cnv[Adding an {virt} source provider] or xref:adding-source-provider_dest_cnv[Adding {a-virt} destination provider].
...
+ 
Your plan appears in the table on the *Plans* page, and you can run it from that page.

The *Plan details* page includes six tabs, which are described on the table that follows:

[cols="1,1,1,1,1", options="header"]
.Tabs of the Plan details page
|===
|Details |YAML|Virtual Machines |Resources |Mappings|Hooks
|Described in the procedure |Lets you add YAML or JSON plan definitions or upload a file containing one them |The virtual machines the plan migrates |?? |??
|===
