...
[role="_abstract"]
Warm migration operates in two stages: the precopy stage, where most data is copied while virtual machines (VMs) run, and the cutover stage, where VMs are shut down to complete the migration.

[id="warm-migration-precopy_{context}"]
== Precopy stage
...
A VM can support up to 32 CBT snapshots. If the source VM has too many CBT snapshots and the Migration Controller service cannot create a new snapshot, warm migration might fail. The Migration Controller service deletes each snapshot when the snapshot is no longer required.

The precopy stage runs until the cutover stage is started manually or is scheduled to start.

[id="warm-migration-cutover_{context}"]
== Cutover stage
