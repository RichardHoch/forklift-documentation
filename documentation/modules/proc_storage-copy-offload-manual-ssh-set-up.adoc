...
. Configure the SSH clone method in your `Provider` CR by setting `settings:esxiCloneMethod` to `"ssh"`.
+
.Example Provider CR
...
. Get the public key from the auto-generated secret by performing the following steps:
