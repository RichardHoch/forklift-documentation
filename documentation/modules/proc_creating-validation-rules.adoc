...
----
+
For information about Rego files, see xref:con_about-rego-files_{context}[About Rego files].
...
Update the inventory rules version after creating a custom rule so that the `Provider Inventory` service detects the changes and validates the VMs. For more information, see xref:proc_updating-inventory-rules-version_{context}[Updating the inventory rules version].
