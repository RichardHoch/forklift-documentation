...
** Disabled SSH
+
*Solution*: Manually enable an SSH connection on the ESXi host by using the commands in link:{mtv-plan}assembly_planning-migration-vmware_mtv#proc_storage-copy-offload-manual-ssh-set-up_vmware[Setting up storage copy offload using manually generated SSH keys].
...
----
$ {oc} edit secret <provider_secret_name> -n {namespace}
----
+
...
. Set a management interface for the SVM and enter its `hostname` in the `STORAGE_HOSTNAME` parameter by following the instructions in the NetApp Knowledge Base article, link:https://kb.netapp.com/Cloud/Astra/Trident/Trident_fails_to_access_ONTAP_on_SVM_level_and_on_Cluster_level[Trident fails to access ONTAP on SVM level and on Cluster level]. The link requires you to log in.
