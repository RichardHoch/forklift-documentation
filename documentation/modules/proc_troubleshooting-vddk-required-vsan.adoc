...
. Create a VDDK image. For more information, see link:{mtv-plan}assembly_provider-specific-requirements-for-migration_mtv#proc_creating-vddk-image_mtv[Creating a VDDK image].

. Upload the VDDK image to the cluster:
+
.. Verify that the image is uploaded to the +{namespace}+ namespace or the provider's namespace.
