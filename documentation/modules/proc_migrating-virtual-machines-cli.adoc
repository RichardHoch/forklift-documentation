...
`<network_attachment_definition_namespace>`::
Required only when `type` is `multus`. Specifies the namespace of the {virt} NAD.
...
`<network_attachment_definition_namespace>`::
Required only when the `type` is `multus`. Specifies the namespace of the {virt} NAD. 
endif::[]
