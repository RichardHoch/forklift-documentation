...
* In {project-short} 2.7.11, you can now migrate shared disks from VMware vSphere by using the new parameter `migrateSharedDisks` in `Plan` CRs.
+
...
[NOTE]
====
This parameter applies only to cold migrations. Warm migration of shared disks is not supported. 
====
