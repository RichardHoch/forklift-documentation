...
+
`convertorLabels`, `convertorNodeSelector`, and `convertorAffinity` are labels that support importer pod scheduling, a feature that lets you direct {project-short} to migrate {vmw} VMs using specific `virt-v2v` convertor pods (importer pods). The feature is for cold migrations only.
+
