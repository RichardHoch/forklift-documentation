...
`convertorLabels`::
Specifies organizational or operational labels for importer pods for identification and management. This is an optional label.

`convertorNodeSelector`::
Specifies the key-value pairs that must be matched for data to be transferred by the importer pods to the specified target nodes. This is an optional label.

`convertorAffinity`::
Specifies an importer pod affinity rule, which may be quite complex, that is entered in the lines following this label. This is an optional label.
+
`convertorLabels`, `convertorNodeSelector`, and `convertorAffinity` are labels that support importer pod scheduling, a feature that lets you direct {project-short} to migrate virtual machines (VMs) using specific importer pods. 
// For more information on the feature in general, see xref:target-vm-scheduling-options_mtv[Target VM scheduling options]. For more details on using the feature with the CLI, including an example YAML snippet, see xref:configuring-target-vm-scheduling-cli_mtv[Scheduling target VMs from the command-line interface]. GET INFO FROM Kobi's PR.
