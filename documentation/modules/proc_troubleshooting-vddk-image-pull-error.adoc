...
[role="_abstract"]
When you create a provider in a namespace different from +{namespace}+, migrations fail with an image pull error.

The VDDK init image URL is located in the +{namespace}+ namespace. If you create a provider in a different namespace, there is an error when pulling the image.
...
----
+
.. If necessary, create a service account with access to pull images from the +{namespace}+ namespace.
+
