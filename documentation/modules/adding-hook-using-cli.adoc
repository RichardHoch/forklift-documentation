...
spec:
  ...
  playbook: |
    <Base64-encoded_Ansible_Playbook>
EOF
----
+
where:
...
<service account>::
(Optional) {ocp} service account. The `serviceAccount` must be provided if you want to manipulate any resources of the cluster.

Base64-encoded Ansible Playbook::
For example, `LS0tCi0gbm...`. If you specify a playbook, the `image` must include an `ansible-runner`.
+
[NOTE]
====
