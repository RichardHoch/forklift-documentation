...
[role="_abstract"]
{project-full} uses `virt-v2v` convertor pods, or _importer pods_, to transfer data from VMware source virtual machines (VMs) to target VMs. By default, {virt} assigns the nodes to which these importer pods tranfer data. However, for cold migrations from VMware VMs, you can schedule the destination nodes for the importer pods.
