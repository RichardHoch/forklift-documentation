...
// * documentation/doc-Migrating_your_virtual_machines/assemblies/assembly_migrating-from-vmware.adocw
:_mod-docs-content-type: REFERENCE
[id="ref_migration-plan-options-ui_{context}"]
= Migration plan options

[role="_abstract"]
On the *Migration plans* page of the {ocp} web console, you can click the {kebab} beside a migration plan to access the following options:
...
ifdef::vmware,rhv[]
* *Set cutover*: Active only for warm migrations. Clicking *Set cutover* opens the *Set cutover* window, which allows you to set the date and time for a cutover.
* *Edit cutover*: Change the date or time of a scheduled cutover. Active only for warm migrations. Clicking *Edit cutover* opens the *Edit cutover* window, which allows you to set a new date or time for a cutover.
endif::vmware,rhv[]
