...
In earlier releases of {project-short}, the main controller container of `forklift-container` crashed during VMware migrations because users created snapshots of the VMs during migrations. As a result, some virtual machines (VMs) migrated, but others failed to migrate. This issue has been resolved in {project-short} 2.7.11, but users are cautioned not to create snapshots of VMs during migrations. link:https://issues.redhat.com/browse/MTV-2164[(MTV-2164)]
