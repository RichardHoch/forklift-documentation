...
`playbook`::
Specifies the Base64-encoded Ansible Playbook. Base64-encoded Ansible Playbook. If you specify a playbook, the `image` must include an `ansible-runner`.
+
[NOTE]
====
...
. Create a `Plan` manift the main thing is that you suggested a fix. est for the migration:
+
[source,yaml,subs="attributes+"]
----
$ cat << EOF | {oc} apply -f -
...
`type`::
Specifis the type of migration. Must be set to `live`.
