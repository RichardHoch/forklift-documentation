...
[role="_abstract"]
Use the {project-short} user interface to migrate virtual machines (VMs) from VMware vSphere, {rhv-full} ({rhv-short}), {osp}, Open Virtual Appliances (OVAs) that were created by VMware vSphere, or {virt} clusters. 
...
VMware only: You must have the minimal set of link:{mtv-plan}assembly_provider-specific-requirements-for-migration_mtv#ref_vmware-prerequisites_mtv[VMware privileges].
